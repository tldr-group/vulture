...
expr_path = "vulture/models/configs/upsampler_AU_compressed.json"
expr = expriment_from_json(expr_path)
copy(expr_path, f"{OUT_PATH}/config.json")
print(expr)

train_ds = EmbeddingDataset(
    "data/imagenet_reduced", "train", expr=expr, device=DEVICE, data_suffix="_au", using_splits=False
)
val_ds = EmbeddingDataset(
    "data/imagenet_reduced", "val", expr=expr, device=DEVICE, data_suffix="_au", using_splits=False
)
